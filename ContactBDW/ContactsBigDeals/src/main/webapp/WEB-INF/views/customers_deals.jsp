...
function save_values(){
	...
	while ((flag == 1) && (i < 1000)) {
		...
		(document.forms["myform"][nIdCustomer].style.color == 'red' ||
...
function new_row(){
	...
	cell2.innerHTML = '<input type="text" class="id_customer" name="id_customerS' + minusI + '" value="${pId}" disabled>';
	cell3.innerHTML = '<input type="text" class="product_name" name="name_goodS' + minusI + '" value="">';
	...
	cell8.innerHTML = '<input type="text" class="dt" name="date_dealS' + minusI + '" value="TODAY">';

	// send data to MvcController without refreshing page
	var http = new XMLHttpRequest();
	http.open("POST", "/customers_deals_insert", true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	var params='p1='+'${pId}';
	http.send(params);
...
function delete_row(i){
	...
	var http = new XMLHttpRequest();
	http.open("POST", "/customers_deals_delete", true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	...
	document.forms["myform"][nId].value = "";
	document.forms["myform"][nIdCustomer].value = "";
...
//****************************************************************************
    ...
    <div align="center">
		<br>
        <h1>Customer Deal List</h1>
		<br>
		<h2>${nameCustomer} - ${typeCustomer}</h2>
		<br>
        <form name="myform" >
            <table  name="mytable" id="mytable" border="1" width="600px" height="50px"  style="width:800px">
				...
				<c:forEach var="sup" items="${listCustomers_deals}" varStatus="i">
                    <tr>

                        <td><input type="text" class="id" name="id${i.index}" value="${sup.id}"
                        	maxlength="7" size="7" disabled></td>
                        <td><input type="text" class="id_customer" name="id_customer${i.index}" value="${sup.id_customer}"
                        	onchange="myChangeFunction('id_customer${i.index}')" disabled></td>
                        <td><input type="text" class="product_name" name="name_good${i.index}" value="${sup.name_good}"
                        ...
                        <td><input type="text" class="dt" name="date_deal${i.index}" value="${sup.date_deal}"
                        	onchange="myChangeFunction('date_deal${i.index}')"  disabled></td>
		...
		<br>
        <input type="button" value="Save" name="buttonSubmit" onclick="return save_values()">
        <input type="button" value="+ Add" name="buttonAdd" onclick="return new_row()">
		</form>
    </div>
	<br>
