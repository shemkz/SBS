	...
	while ((flag == 1) && (i < 1000)) {
		...
		{//.style.color == 'red' - changed
			...
			if (p1==null || p1=="") {
				...
				http.open("POST", "/suppliers_save", true);
...
function new_row(){
	...
	cell4.innerHTML = '<input type="text" class="name_ml" name="middle_nameS' + minusI + '" value="">';
	cell5.innerHTML = '<input type="text" class="name_ml" name="last_nameS' + minusI + '" value="">';
	...
	var http = new XMLHttpRequest();
	http.open("POST", "/suppliers_insert", true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	...
	http.onload = function() { 
		...
		cell12.innerHTML = `<input type="button" value="Deals..." 
                        	onclick="window.location.replace('/suppliers_deals?pId=` + newId.trim() + `');">`;
		}//  http.onload            
}
...
function delete_row(i){
	...
	var http = new XMLHttpRequest();
	http.open("POST", "/suppliers_delete", true);
	http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
        ...
        <form name="myform" >
            <table  name="mytable" id="mytable" border="1" width="600px" height="50px"  style="width:1100px">
                <tr>
					...
					<c:forEach var="sup" items="${listSuppliers}" varStatus="i">
                    <tr>
                        ...
                        <td><input type="text" class="name_ml" name="middle_name${i.index}" value="${sup.middle_name}"
                        	onchange="myChangeFunction('middle_name${i.index}')"></td>
                        <td><input type="text" class="name_ml" name="last_name${i.index}" value="${sup.last_name}"
                        ...
                        <td><input type="button" value="Deals..." 
                        	onclick="window.location.replace('/suppliers_deals?pId=${sup.id}');"></td>
